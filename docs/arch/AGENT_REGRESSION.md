...
- [ ] **B1.2 — Live: gecachter, jetzt fremd-belegter Port wird nicht zyklt** · `P1`
  ...
  - **Assert:** `N.guard("staleb1")` → state `escalated`. Im `N.log()`: WARNING-Marker `last-known port 'PB1' for 'aa:aa' now serves 'bb:bb' — dropping stale cache` UND der Block-Marker des poe_port-Drivers `PoE aa:aa: no port matches 'aa:aa'` (core/drivers/poe_port.py:39, `can_recover` blockt VOR `repair()` → Engine eskaliert via `recovery_blocked`). Der `repair()`-Marker `cannot repair 'aa:aa'` wird NICHT erwartet (repair wird nie erreicht). Kein erfolgreicher Cycle des Ports nach dem Umkabeln.
...
- [ ] **LINK-1 — Link-Checkboxen symmetrisch (Add + Reconfigure)** · `P1`
  ...
  - **Files:** `core/links.py` → `link_components`/`group_of` (ungerichteter Union + Connected-Components, stale ids werden verworfen); `config_flow.py:849` → `group_of(...)` liefert die Section-Defaults; `config_flow.py:243/396` → `SECTION_LINK`/`CONF_LINKED_GUARDS`-Selector (zeigt nur recover-fähige andere Guards).
...
- [ ] **POE-1 — `resolve_with_reason`: 1 / 0 / >1 Match** · `P0`
  ...
  - **Files:** `core/poe.py:151` → `resolve_with_reason` (`len(live)==1` → `_learn`+return; `>1` → `f"'{identifier}' matches {len(live)} ports"`; cache-Fallback; sonst `f"no port matches '{identifier}'"`); DEBUG-Trace `core/poe.py:163` `"PoE %s:   port %r reports id %r"`.
...
- [ ] **POE-2 — Stale-Cache-Drop bei umgekabeltem Port (B1)** · `P0`
  ...
  - **Files:** `core/poe.py:177-198` → `resolve_with_reason` (Cache-Fallback: `occupant = _norm(self._port_id(port))`; `occupant is None` → last-known WARNING + return; sonst `"now serves %r — dropping stale cache"` + `_cache.pop(target, None)` + refuse).
...
- [ ] **POE-3 — Coalescing statt Per-Port-Lock: Driver + Service teilen EINEN Cycle** · `P0`
  ...
  - **Files:** `core/poe.py:231-257` → `repair` (`self._inflight[label]`; bei laufendem Task `"already recovering — joining in-flight cycle"` + `await asyncio.shield(task)`; sonst Task synchron registrieren), `_run_cycle`/`_cycle`. **Kein** `asyncio.Lock` mehr.
...
- [ ] **POE-4 — Service `repair_poe_port` heilt eigenständig + Status-Event** · `P1`
  ...
  - **Files:** `core/poe.py:231` → `repair`; `core/poe.py:259-266` → `_run_cycle` (`_set_status` PORT_RECOVERING→good/failed); `core/poe.py:222-228` → `_set_status` feuert `EVENT_PORT_STATUS = f"{DOMAIN}_poe_port"` (**definiert in `core/poe.py:44`, nicht const.py**); Service-Registrierung in `__init__.py:134-140` (`_repair_poe_port`).
...
- [ ] **POE-5 — `poe_port`-Driver = dünner Fabric-Adapter (kein Per-Guard-Cache)** · `P1`
  ...
  - **Files:** `core/drivers/poe_port.py:29` → `_fabric()` (`hass.data.get(DOMAIN, {}).get("fabric")`), `can_recover`→`fabric.resolve_with_reason`, `recover`→`fabric.repair`, `config_errors`→`fabric.port_count==0`.
...
- [ ] **POE-6 — Platzhalter-ids werden nie gelernt (kein „moved port"-Sturm)** · `P0`
  ...
  - **Files:** `core/poe.py:48` → `_PLACEHOLDER_IDS`, `core/poe.py:51` → `_norm` (collapse to None), `core/poe.py:120-125` → `_relearn` (nur bei `pid` truthy → `_learn`), `resolve_with_reason` (`target=_norm(identifier)`).
...
- [ ] **POE-7 — Re-Cabling-WARNING nur bei echtem Wechsel einer realen id** · `P2`
  ...
  - **Files:** `core/poe.py:109-118` → `_learn` (`prev is None` → INFO `"learned"`; sonst WARNING `"%r moved port %r -> %r"`).
...
- [ ] **HQ4 — state_based end-to-end: Guard reagiert auf Health-Entity** · `P0`
  ...
  - **Files:** `core/health/entity_state.py` → `watched_entities` (Z. 54-56), `evaluate` (Z. 58-81).
...
- [ ] **HQ5 — template_based + Tracking re-evaluiert bei Referenz-Änderung** · `P0`
  ...
  - **Files:** `core/health/template.py` → `async_setup` (Z. 61-72), `watched_entities` (Z. 39-41), `evaluate` (Z. 50-59).
...
- [ ] **HQ6 — Template-Verdicts: kein Fehlalarm bei unklarem Ergebnis** · `P0`
  ...
  - **Files:** `core/health/template.py` → `evaluate` Z. 50-59, `_UNKNOWN_RESULTS={"","none","unknown","unavailable"}` (Z. 29).
...
- [ ] **HQ-STATE-UNK — state_based: unavailable/unknown → UNKNOWN, kein Recover** · `P1`
  ...
  - **Files:** `core/health/entity_state.py` → `evaluate` Z. 75-76 (`actual in (STATE_UNAVAILABLE, STATE_UNKNOWN) → UNKNOWN`), Z. 69-73 (expliziter `off_value` würde gewinnen).
...
- [ ] **ST2 — switch: off→delay→on Power-Cycle** · `P0`
  ...
  - **Files:** `config_flow.py` → `_build_driver` Z. 534-538 (`switch_cycle`), `core/drivers/switch_cycle.py` → `recover` (Z. 29-42).
...
- [ ] **ST4 — actions: getrennte Aus-/Ein-Aktion + Delay** · `P0`
  ...
  - **Files:** `config_flow.py` → `_build_driver` Z. 527-533 (`action_cycle`), `core/drivers/action_cycle.py`.
...
- [ ] **ST5 — poe_port: Resolver findet Port per expected_id** · `P0`
  ...
  - **Files:** `config_flow.py` → `_build_driver` Z. 523-524 (`poe_port`,`expected_id`); `core/drivers/poe_port.py` → `can_recover` Z. 33-41 (`fabric.resolve_with_reason`), `target_info` Z. 52-56, `config_errors` Z. 58-65.
...
- [ ] **NA4 — Fehlender Notify-Service → gefangen, kein Crash** · `P0`
  ...
  - **Files:** `core/notify.py:45` `_run` fängt `vol.Invalid` (`"Notify action invalid for …"` Zeile 49) bzw. `Exception` (`LOGGER.exception("Notify action failed for …")` Zeile 51); detached via `hass.async_create_task` (Zeile 53).
...
- [ ] **CC2 — Template-Verdicts: UNKNOWN macht keinen Fehlalarm** · `P0`
  ...
  - **Files:** `core/health/template.py:29` `_UNKNOWN_RESULTS={"","none","unknown","unavailable"}`; `evaluate()` (Zeile 50): `TemplateError`→UNKNOWN (Zeile 54), `None`/leer/none-String→UNKNOWN (Zeile 55–58), sonst `result_as_boolean` (Zeile 59).
...
- [ ] **CC3 — Health-Entität unavailable/unknown (state_based) → UNKNOWN, kein Recover** · `P0`
  ...
  - **Files:** `core/health/entity_state.py:75` ambivalente States (`unavailable`/`unknown`) → UNKNOWN; Zeile 72: explizites `off_value` gewinnt (auch über unavailable).
...
- [ ] **CC5 — poe_port: kein/mehrdeutiger Match → blockt → ESCALATED `recovery_blocked`** · `P0`
  ...
  - **Files:** `core/drivers/poe_port.py:33` `can_recover` → `fabric.resolve_with_reason` → `port is None` → `LOGGER.error("PoE %s: %s")` (Zeile 39) + `return False, reason`; `core/poe.py:198` Laufzeit-Reason `"no port matches '<id>'"`; `core/drivers/poe_port.py:58` `config_errors` „no ports configured" wenn `port_count==0` (Startup-Check via `engine._check_config`).
...
- [ ] **MS1 — Health on/off als Listen: booting∈on→OK, error∈off→UNHEALTHY** · `P1`
  ...
  - **Files:** `config_flow.py` → `_HEALTH_VALUE_SELECTOR=_LiveStateSelector(...,multiple=True)` (Z.194/215), `_watch_fields` (Z.225-233) baut `on_value`/`off_value` als Listen; Auswertung in `core/health/entity_state.py` → `evaluate`: `off`-Treffer→`UNHEALTHY`, `on`-Treffer→`OK`, sonst `UNKNOWN`.
...
- [ ] **CFG-3 — poe_port ohne Ports → ERROR `no ports configured`** · `P0`
  - **Prüft:** Ein `poe_port`-Guard loggt einen Config-Error, wenn die Fabric keine Ports kennt (core/drivers/poe_port.py `config_errors` Z. 58–65 → `fabric.port_count == 0`).
  - **Files:** core/drivers/poe_port.py Z. 58–65 — Marker `"no ports configured — add ports in the integration's options"`.
...
- [ ] **NOT1 — Logs Englisch, Notify-Meldung Deutsch (language=de)** · `P1`
  ...
  - **Files:** `core/notify.py` → `async_notify` Zeile 32–34 (`lang=...; messages=NOTIFY_MESSAGES.get(lang,...)`); `const.py` → `NOTIFY_MESSAGES["de"]` Zeile 159–168.
...
- [ ] **NOT2 — Notify-Aktion mit defektem Service → gefangen, kein Crash** · `P1`
  ...
  - **Files:** `core/notify.py` → `_run` Zeile 45–53 (`except vol.Invalid` → `"Notify action invalid for %s"` / `except Exception` → `"Notify action failed for %s"`).
...
- [ ] **GAP-B1a — Stale-Cache via Re-Cabling-Simulation (Live)** · `P1`
  ...
  - **Files:** `core/poe.py` → `resolve_with_reason` Zeile 177-198 (occupant-Check: `occupant is None` ⇒ last-known erlaubt; sonst `pop(target)` + WARNING `"now serves … — dropping stale cache"`).
...
- [ ] **GAP-B1b — last-known greift nur bei leerem Port** · `P1`
  ...
  - **Files:** `core/poe.py` → `resolve_with_reason` Zeile 183-190 (`occupant is None` ⇒ WARNING `"not in any port's neighbour data — last-known port"`, return port).
...
- [ ] **GAP-PE1 — PoE-Cache in den Store persistiert (_poe_cache)** · `P1`
  ...
  - **Files:** `__init__.py` → `_serialize` Zeile 116-119 (`data["_poe_cache"] = fabric.cache`); `fabric.set_ports(ports, cache=stored.get("_poe_cache"))` Zeile 128; `core/poe.py` → `set_ports` `cache`-Param Zeile 71-75.
...
- [ ] **GAP-PE2 — Placeholder-Ids werden nie gelernt (kein WARNING-Storm)** · `P2`
  ...
  - **Files:** `core/poe.py` → `_PLACEHOLDER_IDS` Zeile 48, `_norm` Zeile 51-56, `_relearn` Zeile 120-125; Unit-Test `tests/test_poe.py::test_placeholder_ids_are_never_learned`.
...
- [ ] **GAP-LK1 — notify-only-Guard fällt aus allen Link-Gruppen (Closure schließt ihn aus)** · `P1`
  ...
  - **Files:** `__init__.py` → `_is_recover` Zeile 148-152 + `device_ids`/`declared_links` Zeile 154-160; `core/links.py` → `link_components` (stale ids via `valid` gedroppt).
...
- [ ] **GAP-CC4 — _await_status: bereits-im-Zielzustand-Port (kein Timeout-Warten)** · `P2`
  ...
  - **Files:** `core/poe.py` → `_await_status` Zeile 294-337 (`if current() in targets: return True` Zeile 312-313, vor dem Event-Abonnement).
...
- [ ] **GAP-CC5 — Ambiguer Resolve verweigert (>1 Live-Match)** · `P2`
  ...
  - **Files:** `core/poe.py` → `resolve_with_reason` Zeile 172-176 (`len(live) > 1` ⇒ `None, "matches N ports"`); Unit `test_poe.py::test_resolve_ambiguous`.
...
- [ ] **GAP-CC6 — Coalescing ersetzt per-Port-Lock (richtige Test-Namen)** · `P1`
  ...
  - **Files:** `core/poe.py` → `repair` Zeile 231-257 (`self._inflight`, `asyncio.shield(task)`), KEIN `asyncio.Lock`; Tests `test_concurrent_callers_coalesce` + `test_driver_and_service_coalesce` (NICHT `*_per_port_lock_*`).
